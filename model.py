...
import os
device = "cuda:0" if torch.cuda.is_available() else "cpu"   #可先确认是否有gpu

ini_params = {
    'device': device,  # 设备类型，带GPU选择GPU
    'conf': 0.25,  # 物体置信度阈值
...
def count_classes(det_info, class_names):
    ...
    :param det_info: 检测信息列表，每个项目都是类似的列表，如 [class_name, bbox, conf, class_id]
    :param class_names: 所有可能的类名列表
    :return: 每个类别计数的列表
    """
